...
final class PairingAndRuntimeTests: XCTestCase {
    ...
    func testAutomaticDeviceProfileIncludesRuntimeAndCoarseNetworkShape() throws {
        ...
        XCTAssertFalse(profile.device?.locale?.contains("_") == true)
        XCTAssertFalse(profile.device?.language?.isEmpty == true)
        XCTAssertNotNil(profile.device?.timeZone)
        XCTAssertNotNil(profile.device?.utcOffsetMinutes)
