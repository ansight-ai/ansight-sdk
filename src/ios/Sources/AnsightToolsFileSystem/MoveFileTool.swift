...
public final class MoveFileTool: AnsightTool {
    ...
    public var descriptor: AnsightToolDescriptor {
        AnsightToolDescriptor(
            id: AnsightFileSystemToolIds.moveFile,
            name: "Move File",
    ...
    public func execute(arguments: [String: String]) throws -> AnsightToolExecutionResult {
        do {
            ...
            if source.fullPath == destination.fullPath {
                throw AnsightFileSystemToolError.invalidArgument("The source and destination paths must be different.")
            }
            ...
            try FileManager.default.moveItem(atPath: source.fullPath, toPath: destination.fullPath)
            ...
            return .failure(error.localizedDescription, errorCode: "filesystem_move_failed")
        }
    }
    ...
    private func transferResult(
        ...
        guard case .object(var payload) = AnsightFileSystemContentDescriptor.resolvedFilePayload(
            ...
            return .failure("Failed to build move payload.", errorCode: "filesystem_move_failed")
        }
