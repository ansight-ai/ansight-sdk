...
public final class AnsightFlutterPlugin: NSObject, FlutterPlugin, AnsightNativeHostApi {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        let messenger = registrar.messenger
        #endif
        instance.dartApi = AnsightDartApi(binaryMessenger: messenger)
    ...
    private func dispatch(method: String, arguments: NSDictionary) async throws -> Any {
        ...
        case "submitFlutterScreenFrame":
            guard let pngBase64 = stringValue(arguments, "pngBase64"),
                  ...
                  let jpegData = encodeJpeg(
                    imageData: pngData,
                    quality: intValue(arguments, "quality", defaultValue: 60)
                  )
            else {
                return operationResultDictionary(
                    .failure("Flutter screen frame could not be decoded or encoded as JPEG.")
                )
            }
            let visualTrees = (arguments["visualTrees"] as? [Any])?
                .compactMap(jsonValue) ?? []
            return operationResultDictionary(
                await AnsightRuntime.shared.submitScreenFrame(
                    jpegData: jpegData,
                    width: intValue(arguments, "width", defaultValue: 0),
                    height: intValue(arguments, "height", defaultValue: 0),
                    quality: intValue(arguments, "quality", defaultValue: 60),
                    visualTrees: visualTrees
                )
            )
        case "submitFlutterPointerEvent":
            return operationResultDictionary(
                AnsightRuntime.shared.submitTouchInput(
                    action: stringValue(arguments, "action") ?? "",
                    pointerId: Int64(intValue(arguments, "pointerId", defaultValue: 0)),
                    x: doubleValue(arguments, "x", defaultValue: .nan),
                    y: doubleValue(arguments, "y", defaultValue: .nan),
                    surfaceWidth: doubleValue(arguments, "surfaceWidth", defaultValue: 0),
                    surfaceHeight: doubleValue(arguments, "surfaceHeight", defaultValue: 0),
                    surfaceScale: doubleValue(arguments, "surfaceScale", defaultValue: 0)
                )
            )
...
private func encodeJpeg(imageData: Data, quality: Int) -> Data? {
    ...
    let compression = CGFloat(max(1, min(quality, 100))) / 100.0
