...
object Runtime {
    ...
    fun updateCustomProperties(customProperties: Map<String, Map<String, String>>): OperationResult {
        return if (isInitialized()) {
            AnsightRuntime.updateCustomProperties(customProperties)
        } else {
            OperationResult.failure("AnsightRuntime is not initialized.")
        }
    }

    @JvmStatic
    fun UpdateCustomProperties(customProperties: Map<String, Map<String, String>>): OperationResult {
        return updateCustomProperties(customProperties)
    }

    @JvmStatic
    fun registerCustomProperty(group: String, key: String, value: String): OperationResult {
        return if (isInitialized()) {
            AnsightRuntime.registerCustomProperty(group, key, value)
        } else {
            OperationResult.failure("AnsightRuntime is not initialized.")
        }
    }
    ...
    @JvmStatic
    fun RegisterCustomProperty(group: String, key: String, value: String): OperationResult {
        return registerCustomProperty(group, key, value)
    }

    @JvmStatic
    fun removeCustomProperty(group: String, key: String): OperationResult {
        return if (isInitialized()) {
            AnsightRuntime.removeCustomProperty(group, key)
        } else {
            OperationResult.failure("AnsightRuntime is not initialized.")
        }
    }
    ...
    @JvmStatic
    fun RemoveCustomProperty(group: String, key: String): OperationResult {
        return removeCustomProperty(group, key)
    }

    @JvmStatic
    fun clearCustomProperties(): OperationResult {
        return if (isInitialized()) {
            AnsightRuntime.clearCustomProperties()
        } else {
            OperationResult.failure("AnsightRuntime is not initialized.")
        }
    }
