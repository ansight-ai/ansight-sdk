...
object AndroidStandardTools {
    fun create(): List<AndroidTool> = buildList {
        add(uiTool("ui.get_visual_tree", "Get Visual Tree", "Returns the current Android View hierarchy.") { _, _ ->
            AndroidToolResult.success(AndroidUiEvidence.visualTree())
        })
        add(uiTool("ui.get_screenshot", "Get Screenshot", "Captures a screenshot of the current Android activity.") { args, context ->
        ...
        add(uiTool("ui.inspect_node", "Inspect Node", "Returns one node from the current visual tree.") { args, _ ->
        ...
        add(uiTool("ui.show_overlay", "Show Overlay", "Shows a rectangular diagnostic overlay.", ToolScope.Write) { args, _ ->
            AndroidToolResult.success(AndroidUiEvidence.showOverlay(args))
        })
        add(uiTool("ui.get_overlay", "Get Overlay", "Returns one diagnostic overlay.") { args, _ ->
        ...
        add(uiTool("ui.query_overlays", "Query Overlays", "Lists active diagnostic overlays.") { _, _ ->
            AndroidToolResult.success(AndroidUiEvidence.queryOverlays())
        })
        add(uiTool("ui.update_overlay", "Update Overlay", "Updates a diagnostic overlay.", ToolScope.Write) { args, _ ->
            AndroidToolResult.success(AndroidUiEvidence.updateOverlay(args))
        })
        add(uiTool("ui.remove_overlay", "Remove Overlay", "Removes a diagnostic overlay.", ToolScope.Delete) { args, _ ->
            val id = args["id"] ?: return@uiTool AndroidToolResult.failure("Overlay id is required.", "overlay_id_required")
            AndroidToolResult.success(AndroidUiEvidence.removeOverlay(id))
        })
        add(uiTool("ui.clear_overlays", "Clear Overlays", "Clears diagnostic overlays.", ToolScope.Delete) { _, _ ->
            AndroidToolResult.success(AndroidUiEvidence.clearOverlays())
        })

        add(fileTool("files.list_directory", "List Directory", "Lists files inside an approved app root.") { args, context ->
        ...
        add(fileTool("files.read_file", "Read File", "Reads a UTF-8 text file inside an approved app root.") { args, context ->
        ...
        add(fileTool("files.get_file_checksum", "Get File Checksum", "Returns a SHA-256 checksum for a file.") { args, context ->
            val file = FileSandbox.resolve(context.application, args, requireExisting = true, expectDirectory = false)
        ...
        add(fileTool("files.download_file", "Download File", "Returns a small file inline as base64.") { args, context ->
            val file = FileSandbox.resolve(context.application, args, requireExisting = true, expectDirectory = false)
        ...
        add(fileTool("files.begin_binary_download", "Begin Binary Download", "Transfers a file over the WebSocket binary channel.") { args, context ->
            val file = FileSandbox.resolve(context.application, args, requireExisting = true, expectDirectory = false)
        ...
        add(fileTool("files.push_file", "Push File", "Writes a file inside an approved app root.", ToolScope.Write) { args, context ->
        ...
        add(fileTool("files.copy_file", "Copy File", "Copies a file between approved app roots.", ToolScope.Write) { args, context ->
        ...
        add(fileTool("files.move_file", "Move File", "Moves a file between approved app roots.", ToolScope.Write) { args, context ->
            val source = FileSandbox.resolve(context.application, args, requireExisting = true, expectDirectory = false)
            val destination = FileSandbox.resolve(context.application, args, pathKey = "destinationPath", rootKey = "destinationRoot", requireExisting = false, expectDirectory = false)
            destination.file.parentFile?.mkdirs()
        ...
        add(fileTool("files.delete_file", "Delete File", "Deletes a file inside an approved app root.", ToolScope.Delete) { args, context ->
            val file = FileSandbox.resolve(context.application, args, requireExisting = true, expectDirectory = false)
            val deleted = file.file.delete()
            AndroidToolResult.success(FileSandbox.describe(file).put("deleted", deleted))
        })

        add(prefsTool("prefs.list_keys", "List Preference Keys", "Lists keys from SharedPreferences.") { args, context ->
        ...
        add(prefsTool("prefs.get_value", "Get Preference Value", "Reads one SharedPreferences value.") { args, context ->
        ...
        add(prefsTool("prefs.set_value", "Set Preference Value", "Writes one SharedPreferences string value.", ToolScope.Write) { args, context ->
            val key = args["key"] ?: return@prefsTool AndroidToolResult.failure("Preference key is required.", "preference_key_required")
        ...
        add(prefsTool("prefs.remove_key", "Remove Preference Key", "Removes one SharedPreferences key.", ToolScope.Delete) { args, context ->
            val key = args["key"] ?: return@prefsTool AndroidToolResult.failure("Preference key is required.", "preference_key_required")
            val prefs = preferences(context.application, args)
        ...
        add(secureTool("secure.get_value", "Get Secure Storage Value", "Reads an explicitly allow-listed secure value.") { args, context ->
        ...
        add(secureTool("secure.set_value", "Set Secure Storage Value", "Writes an explicitly allow-listed secure value.", ToolScope.Write) { args, context ->
            val key = args["key"] ?: return@secureTool AndroidToolResult.failure("Secure storage key is required.", "secure_key_required")
            if (!context.options.secureStorage.isAllowed(key)) {
                return@secureTool AndroidToolResult.failure("Secure storage key is not allow-listed.", "secure_key_denied")
            }
        ...
        add(secureTool("secure.remove_key", "Remove Secure Storage Key", "Removes an explicitly allow-listed secure value.", ToolScope.Delete) { args, context ->
            val key = args["key"] ?: return@secureTool AndroidToolResult.failure("Secure storage key is required.", "secure_key_required")
            if (!context.options.secureStorage.isAllowed(key)) {
                return@secureTool AndroidToolResult.failure("Secure storage key is not allow-listed.", "secure_key_denied")
            }
        ...
        add(dataTool("data.list_databases", "List Databases", "Lists SQLite database files in app-owned roots.") { _, context ->
        ...
        add(dataTool("data.describe_schema", "Describe Schema", "Describes a SQLite database schema.") { args, context ->
        ...
        add(dataTool("data.query", "Query Database", "Runs a read-only SQLite query.", ToolScope.Read) { args, context ->
...
private object FileSandbox {
    ...
    fun roots(application: Application): Map<String, File> = buildMap {
        put("appData", application.filesDir)
        put("cache", application.cacheDir)
        application.noBackupFilesDir?.let { put("noBackup", it) }
        application.getExternalFilesDir(null)?.let { put("externalFiles", it) }
        application.getDatabasePath("__ansight_probe__").parentFile?.let { put("databases", it) }
    }.mapValues { it.value.canonicalFile }
...
private fun Map<String, String>.booleanArg(name: String, defaultValue: Boolean): Boolean = this[name]?.toBooleanStrictOrNull() ?: defaultValue
