...
internal object AndroidReflectionTools {
    private const val listRootsId = "reflect.list_roots"
    private const val inspectObjectId = "reflect.inspect_object"
    private const val describeTypeId = "reflect.describe_type"
    private const val setMemberValueId = "reflect.set_member_value"
    private const val invokeMethodId = "reflect.invoke_method"
    ...
    fun create(): List<AndroidTool> = listOf(
        ...
        reflectionTool(
            ...
            val type = args["type"]?.trim()?.ifBlank { null }?.let { Class.forName(it) }
                ...
                ?: return@reflectionTool AndroidToolResult.failure("No type or live object was resolved.", "reflect_type_required")
            describeType(type)
        },
        reflectionTool(
            setMemberValueId,
            ...
            val member = args["member"] ?: args["name"] ?: return@reflectionTool AndroidToolResult.failure("Member name is required.", "reflect_member_required")
            val target = resolveTarget(context, args)
            val receiver = target.value ?: return@reflectionTool AndroidToolResult.failure("Cannot set a member on null.", "reflect_null_target")
            val rawValue = args["value"] ?: return@reflectionTool AndroidToolResult.failure("Value is required.", "reflect_value_required")
        ...
        reflectionTool(
            ...
            val methodName = args["method"] ?: args["name"] ?: return@reflectionTool AndroidToolResult.failure("Method name is required.", "reflect_method_required")
            val target = resolveTarget(context, args)
            val receiver = target.value ?: return@reflectionTool AndroidToolResult.failure("Cannot invoke a method on null.", "reflect_null_target")
            invokeNoArgMethod(receiver, methodName, target.rootId, target.path)
        },
    )

    private fun reflectionTool(
        id: String,
        name: String,
        description: String,
        scope: ToolScope,
    ...
    private fun roots(context: AndroidToolExecutionContext): List<ReflectionRoot> = listOf(
        ReflectionRoot(
            id = "application",
    ...
    private fun writeMember(receiver: Any, member: String, rawValue: String): AndroidToolResult {
        ...
        val setterName = "set" + member.substring(0, 1).toUpperCase(Locale.US) + member.substring(1)
    ...
    private fun readMember(receiver: Any, segment: String): Any? {
        ...
        val value = if (name.isEmpty()) receiver else findField(receiver.javaClass, name)?.let { field ->
            field.isAccessible = true
            field.get(receiver)
        } ?: receiver.javaClass.methods.firstOrNull { it.parameterTypes.isEmpty() && (it.name == name || it.name == "get" + name.substring(0, 1).toUpperCase(Locale.US) + name.substring(1)) }
            ?.let { method ->
                method.isAccessible = true
