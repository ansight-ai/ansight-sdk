...
public sealed class DescribeSchemaTool : ITool
{
    ...
    public Task<ToolResult> Execute(IReadOnlyDictionary<string, string> arguments)
    {
        ...
        try
        {
            ...
            var tableFilter = GetString(arguments, "table");
            return Task.FromResult(ToolResult.Success(DescribeSchema(databasePath, tableFilter)));
        }
    ...
    private static JsonObject DescribeSchema(string databasePath, string? tableFilter)
    {
    ...
    private static JsonArray ExecuteReadOnly(SqliteDatabase database, string sql, int maxRows)
    {
    ...
    private static JsonNode? ReadColumnValue(IntPtr statement, int columnIndex)
    {
        return sqlite3_column_type(statement, columnIndex) switch
        {
            ...
            3 => ReadText(statement, columnIndex),
