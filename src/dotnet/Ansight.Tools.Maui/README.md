...
The visual tree and search tools return MAUI element ids that can be passed to the element, bindable-property, binding, layout, handler, action, wait, XAML experiment, and binding-context tools. Write-scoped tools require `WithReadWriteToolAccess()` or a custom `ToolGuard`.
