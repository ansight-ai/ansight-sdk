...
/// <summary>
/// Renders Ansight metric/event visuals onto a Skia canvas for both sheet and overlay layouts.
/// </summary>
public static class ChartRenderer
{
    ...
    public static RenderResult Render(SKCanvas canvas,
        SKImageInfo info,
        IDataSink dataSink,
        RenderOptions renderOptions)
    {
        ...
        var backgroundColor = renderOptions.Mode == ChartRenderMode.Overlay
        ...
        var cornerScale = renderOptions.Mode == ChartRenderMode.Overlay ? 0.65f : 1f;
        ...
        try
        {
            ...
            foreach (var channelId in visibleChannels)
            {
                ...
                {
                    var isFpsChannel = channelId == Constants.ReservedChannels.FramesPerSecond_Id;
                    if (isFpsChannel)
                    {
                        hasFpsMetrics = true;
            ...
            var layoutScale = renderOptions.Mode == ChartRenderMode.Overlay ? 0.65f : 1f;
            ...
            eventVisuals.Clear();
            var eventBehaviour = renderOptions.AppEventRenderingBehaviour;
            var shouldRenderEvents = eventBehaviour != AppEventRenderingBehaviour.None;
            var shouldRenderLabels = eventBehaviour == AppEventRenderingBehaviour.LabelsAndIcons;
            ...
            {
                var max = channelId == Constants.ReservedChannels.FramesPerSecond_Id
                    ? maxFpsDisplayValue
            ...
            string FormatValue(byte channelId, long value) =>
                channelId == Constants.ReservedChannels.FramesPerSecond_Id
                    ? FormatFps(value)
                    : FormatBytes(value);
            ...
            if (shouldRenderEvents)
            {
                ...
                foreach (var channelId in visibleChannels)
                {
                    var channel = channelLookup.TryGetValue(channelId, out var channelInfo)
                        ? channelInfo
                        : new Channel(channelId, $"Channel {channelId}", Colors.Purple);

                    var channelColor = ToSkColor(channel.Color);
                    var isDetached = channelId == Constants.ReservedChannels.ChannelNotSpecified_Id;
                    ...
                    dataSink.UseEventsInChannelForRange(channelId, fromUtc, toUtc, eventSpan =>
                    {
                        ...
                        foreach (var ansightEvent in eventSpan)
                        {
                            var x = chartRect.Left + (float)((ansightEvent.CapturedAtUtc - fromUtc).TotalMilliseconds / totalMilliseconds) * chartRect.Width;
                            float y;

                            if (useMetrics)
                            {
                                long? metricValue = null;
                                dataSink.UseMetricsInChannelForRange(channelId, fromUtc, toUtc, metricsSpan => { metricValue = GetMetricValueAt(metricsSpan, ansightEvent.CapturedAtUtc); });
                            ...
                            eventVisuals.Add(new EventVisual
                            {
                                X = x,
                                Y = y,
                                AppEventType = ansightEvent.Type,
            ...
            foreach (var span in channelSpans)
            {
                ...
                dataSink.UseMetricsInChannelForRange(channelId, fromUtc, toUtc, metricsSpan =>
                {
                    ...
                    if (channelId == Constants.ReservedChannels.FramesPerSecond_Id)
                    {
                ...
                void DrawFpsGradientLine(ReadOnlySpan<Metric> metricsSpan)
                {
            ...
            if (shouldRenderEvents)
            {
                foreach (var visual in eventVisuals)
                {
                    var iconText = AppEventLegend.GetSymbol(visual.AppEventType);
            ...
            return new RenderResult(chartRect, true);
        }
        finally
        {
            canvas.Restore();
    ...
    private static long? GetMetricValueAt(ReadOnlySpan<Metric> metrics, DateTime targetUtc)
    {
        if (metrics.IsEmpty)
        {
            return null;
    ...
    {
        public static ChartThemePalette For(ChartTheme theme)
        {
            return theme == ChartTheme.Light
                ? new ChartThemePalette(
                    ...
                    new SKColor(250, 67, 31),
                    new SKColor(250, 67, 31),
                    new SKColor(250, 67, 31, 80),
                    new SKColor(255, 255, 255, 235),
                    new SKColor(50, 50, 60),
                    new SKColor(250, 67, 31, 160),
    ...
    private sealed class RenderResources
    {
        ...
        public Dictionary<byte, Channel> ChannelLookup { get; } = new();
        public Dictionary<byte, ChannelSpan> ChannelSpanLookup { get; } = new();
        public List<ChannelSpan> ChannelSpans { get; } = new();
        public List<string> LabelSamples { get; } = new();
        public List<string> FpsLabelSamples { get; } = new();
        public List<Channel> LegendChannels { get; } = new();
    ...
    {
        public LegendEntry(Channel channel, float textWidth)
        {
            Channel = channel;
            TextWidth = textWidth;
            TotalWidth = 0f;
    ...
    private struct EventVisual
    {
        ...
        public AppEventType AppEventType { get; init; }
