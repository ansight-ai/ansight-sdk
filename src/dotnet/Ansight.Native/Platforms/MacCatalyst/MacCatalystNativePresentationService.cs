...
internal sealed class MacCatalystNativePresentationService : IPresentationService
{
    ...
    private NativeChartViewMacCatalyst? overlayChart;

    public MacCatalystNativePresentationService(Options options, IDataSink dataSink, Func<UIWindow?> windowProvider)
    {
    ...
    private void EnsureOverlay(UIWindow window)
    {
        ...
        {
            overlayChart = new NativeChartViewMacCatalyst(new CGRect(0, 0, 320, 180))
            {
                DataSink = dataSink,
...
internal sealed class NativeChartViewMacCatalyst : SKCanvasView
{
    ...
    public NativeChartViewMacCatalyst(CGRect frame) : base(frame)
    {
...
internal sealed class SheetView : UIView
{
    ...
    private readonly NativeChartViewMacCatalyst chart;
    ...
    internal SheetView(CGRect frame, IDataSink dataSink, Options options) : base(frame)
    {
        ...
        closeButton.TouchUpInside += (_, _) => Runtime.DismissSheet();
        ...
        chart = new NativeChartViewMacCatalyst(new CGRect(0, 0, frame.Width, ChartHeight))
        {
            DataSink = dataSink,
