...
internal partial class ShakeGestureListener
{
    ...
    {
        var activity = RuntimePlatform.CurrentActivityProvider?.Invoke();
        if (activity == null)
        {
            Logger.Warning("Shake gesture enable requested but no current Activity is available.");
            return;
        }
        ...
        {
            Logger.Warning("Shake gesture is not supported on this device (no accelerometer).");
            return;
        }
    ...
    private sealed class ShakeSensorListener : Java.Lang.Object, ISensorEventListener
    {
        ...
        private readonly ShakeGestureListener owner;
        private long lastShakeTimestamp;

        public ShakeSensorListener(ShakeGestureListener owner)
        {
            this.owner = owner;
        }
