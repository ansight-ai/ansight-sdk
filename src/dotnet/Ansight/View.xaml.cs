...
/// <summary>
/// Composite view that hosts the Ansight chart, event list, and controls inside the slide-in sheet.
/// </summary>
public partial class View : Grid
{
    ...
    private async void OnSaveSnapshotTapped(object? sender, TappedEventArgs e)
    {
        ...
        {
            Logger.Error("Failed to copy snapshot to clipboard.");
            Logger.Exception(ex);
        }
    }
    ...
    {
        chartView.DataSink = Runtime.Instance.DataSink;
        eventsView.DataSink = Runtime.Instance.DataSink;
    }
