...
/// <summary>
/// Renders Ansight metrics and events as a live chart inside the slide-in sheet or overlay.
/// </summary>
public partial class ChartView : SKCanvasView
{
    ...
    private static void OnDataSinkChanged(BindableObject bindable, object oldValue, object newValue)
    {
        if (bindable is ChartView view)
        {
            if (oldValue is IDataSink oldDataSink)
            {
                view.Unsubscribe(oldDataSink);
            }

            if (newValue is IDataSink newDataSink)
            {
    ...
    {
        var isOverlay = RenderMode == ChartRenderMode.Overlay;
    ...
    private void HandleMetricsUpdated(object? sender, MetricsUpdatedEventArgs e)
    {
        RequestRedraw();
    }
    ...
    private void OnPaintSurface(object sender, SKPaintSurfaceEventArgs e)
    {
        ...
        else
        {
            ...
            var renderOptions = new RenderOptions()
            {
                ...
                CurrentUtc = now,
                Mode = RenderMode,
            ProbePosition = RenderMode == ChartRenderMode.Inline ? probeRatio : null,
        ...
        using var borderPaint = new SKPaint
        {
            ...
            Color = GetBorderColor(Runtime.ChartTheme)
        };
        canvas.DrawRoundRect(roundRect, borderPaint);
    }

    private static SKColor GetBorderColor(ChartTheme theme) => theme == ChartTheme.Light
    ...
    private void UpdateChartBounds(RenderResult renderResult)
    {
