...
- **Native**: React Native exposes the feature through its Android/iOS bridge.
...
| Capability | .NET / MAUI | Android | iOS | React Native |
| --- | --- | --- | --- | --- |
| Initialize, activate, deactivate, and clear the runtime | Yes | Yes | Yes | Native |
| Custom metric channels and metric/event recording | Yes | Yes | Yes | Native |
| Periodically sampled app metric streams | Record with `Runtime.Metric(...)` | `AnsightMetricStream` | `AnsightMetricStream` | No JavaScript sampler API |
| FPS, memory, and battery sampling | Yes | Yes | Yes | Native, plus React Native JS heap channels |
| Screen-view and app-lifecycle events | Yes; automatic in `Ansight.Maui` | Yes | Yes; automatic UIKit capture | Native plus AppState and React Navigation helpers |
| Retained metric/event readback and debug snapshots | Yes | Yes | Yes | Native |
| Device/app profile reporting | Yes | Yes | Yes | Native |
| Live telemetry, event, touch, screenshot, and control streaming | Yes | Yes | Yes | Native |
| App-provided live-session logs | Yes | Yes | Yes | Native |
| Grouped session/custom properties and live mutations | Yes | Yes | Yes | Native |
| Saved pairing config | Yes | Yes | Yes | Native |
| Remembered host profiles and host auto-probe | Yes | Yes | Yes | Native |
| Bundled and developer pairing sources | Yes | Yes | Yes | Native configuration |
| Explicit payload connection | Yes | Yes | Yes | Yes |
| SDK-owned pairing UI | Android/iOS QR via `Ansight.Pairing` | QR scanner and pairing sheet | UIKit file import and QR scanner | No; pass an app-acquired payload to `connect(...)` |
| Connection status, capabilities, and change listeners | Yes | Yes | Yes | Native |
| SDK diagnostic log callbacks | Yes | Yes | Yes | Native listener |
...
| Capability | .NET / MAUI | Android | iOS | React Native |
| --- | --- | --- | --- | --- |
| Periodic live JPEG capture | Yes | Yes | Yes | Native |
| On-demand screenshot (`ui.get_screenshot`; runtime helper where exposed) | Yes | Yes | Yes | Native |
| GPU-backed surface capture option | Accepted for parity | Accepted for parity | Yes | Native; meaningful on iOS |
| Studio-owned simulator/emulator screenshots | Yes | Yes | Yes | Native |
...
| Suite or extension | .NET / MAUI | Android | iOS | React Native |
| --- | --- | --- | --- | --- |
| Custom tool registration and schemas | Yes | Yes | Yes | JavaScript and native |
| Runtime feature package initialization | `IRuntimeFeature` | No | No | No |
| App artifact providers and binary export | Yes | Yes | Yes | JavaScript provider with native binary transport |
| Visual tree, screenshot, inspection, and overlays (`ui.*`) | Yes | Yes | Yes | Native |
| Sandboxed files and binary download (`files.*`) | Yes | Yes | Yes | Native |
| File-descriptor diagnostics (`file_descriptors.*`) | No | Yes | Yes | Native aggregate defaults |
| Preferences (`prefs.*`) | Yes | Yes | Yes | Native |
| Secure storage (`secure.*`) | Yes | Yes | Yes | Native |
| SQLite discovery/schema/read query (`data.*`) | Yes | Yes | Yes | Native |
| Registered-root reflection (`reflect.*`) | Yes | Yes | Yes | Native |
| .NET MAUI inspection and mutation (`maui.*`) | Framework | No | No | No |
| React component/shadow tree and actions (`react.*`) | No | No | No | Framework |
...
| Capability | .NET / MAUI | Android | iOS | React Native |
| --- | --- | --- | --- | --- |
| Generate and embed developer pairing material | MSBuild target; requires a signed source pairing JSON | App/build supplies bundled JSON | SwiftPM/CocoaPods build tool | Configure through the native projects or JS options |
| Detect bundled remote-tool implementations | `AnsightRemoteToolsPolicy` | No SDK build scanner | Build tool; requires `ANSIGHT_ALLOW_REMOTE_TOOLS=true` | Native build rules apply |
| Hard-disable annotated feedback in Release builds | Yes | Not available | Not available | Not available |
