...
- The validator now writes `.ansight-validation/ios-test-app-validation-summary.json`; the current merged strict-gate summary records `totalApps=28`, `studioVerifiedCount=21`, `sdkRuntimeReachedCount=21`, `preRuntimeFailureCount=7`, no Studio verification failures, and passing FPS, screenshot, remote-tool, icon, device-profile, route, touch-input, input-overlay, and binary-download metadata gates for all 21 runtime-reaching rows.
