...
- The validator now writes `.ansight-validation/ios-test-app-validation-summary.json`; the current merged strict-gate summary records `totalApps=2`, `studioVerifiedCount=2`, `sdkRuntimeReachedCount=2`, and passing FPS, screenshot, remote-tool, icon, device-profile, route, touch-input, input-overlay, and binary-download metadata gates for CleanStore and FlappySwift.
