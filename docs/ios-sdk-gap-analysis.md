...
- UDP bootstrap, WebSocket session handoff, remembered host profiles with saved/cached status separation, Wi-Fi-keyed cached profile refresh, legacy cached-profile migration, newest-first cached profile resolution, metric channel announcement, sequence-cursored retained metric/event streaming, app state streaming, clean disconnect, and host status snapshots.
...
1. Host connection parity:
   - Saved config state, cached profile state, .NET precedence order, Wi-Fi-keyed cached profile refresh, legacy cached-profile migration, and newest-first cached profile resolution are implemented.
   - Remaining work is live retry behavior across multiple cached/saved/bundled candidates during a single open-session attempt.
...
- `swift test` in `src/ios`: 63 tests, 1 skipped, 0 failures.
