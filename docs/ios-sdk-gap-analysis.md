...
1. Host connection parity:
   - Saved config state, cached profile state, .NET precedence order, Wi-Fi-keyed cached profile refresh, legacy cached-profile migration, newest-first cached profile resolution, live retry across multiple cached/saved/bundled candidates, and stale cached-profile cleanup are implemented.
...
- `swift test` in `src/ios`: 65 tests, 1 skipped, 0 failures.
