...
- The validator now writes `.ansight-validation/ios-test-app-validation-summary.json`; the current merged strict-gate summary records `totalApps=36`, `studioVerifiedCount=27`, `sdkRuntimeReachedCount=27`, `preRuntimeFailureCount=9`, no Studio verification failures, and passing FPS, screenshot, remote-tool, icon, device-profile, route, touch-input, input-overlay, and binary-download metadata gates for all 27 runtime-reaching rows.
