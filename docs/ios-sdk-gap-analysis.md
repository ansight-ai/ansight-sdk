...
1. Host connection parity:
   - Saved config state, cached profile state, .NET precedence order, Wi-Fi-keyed cached profile refresh, legacy cached-profile migration, newest-first cached profile resolution, live retry across multiple cached/saved/bundled candidates, stale cached-profile cleanup, and app-provided file/QR config reader entry points are implemented.
   - Remaining host pairing polish is a packaged UIKit document picker / QR scanner if native apps need SDK-owned UI instead of supplying their own reader.
...
- `swift test` in `src/ios`: 67 tests, 1 skipped, 0 failures.
