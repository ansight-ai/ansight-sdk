...
- The validator now writes `.ansight-validation/ios-test-app-validation-summary.json`; the current merged strict-gate summary records `totalApps=13`, `studioVerifiedCount=13`, `sdkRuntimeReachedCount=13`, and passing FPS, screenshot, remote-tool, icon, device-profile, route, touch-input, input-overlay, and binary-download metadata gates for all 13 rows.
