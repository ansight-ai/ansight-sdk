...
- The validator now writes `.ansight-validation/ios-test-app-validation-summary.json`; the current merged strict-gate summary records `totalApps=22`, `studioVerifiedCount=15`, `sdkRuntimeReachedCount=15`, `preRuntimeFailureCount=7`, no Studio verification failures, and passing FPS, screenshot, remote-tool, icon, device-profile, route, touch-input, input-overlay, and binary-download metadata gates for all 15 runtime-reaching rows.
